...
def fill_telemetry():
    ...
    for sess in sessions:
        ...
        for drv in drivers:
            ...
            fastest_lap = (
                db.query(Lap) .filter(
                    Lap.driver_id == drv.driver_id,
                    Lap.lap_time_ms.isnot(None),
                    Lap.is_valid) .order_by(
                    Lap.lap_time_ms) .first())
            if not fastest_lap:
            ...
            existing = db.query(TelemetryPoint).filter(
                TelemetryPoint.lap_id == fastest_lap.lap_id).count()
            if existing > 0:
            ...
            # Get telemetry
            try:
                # FastF1 uses driver numbers, try code first
                ...
                for _, pt in tel_s.iterrows():
                    def v(col):
                        val = pt.get(col)
                        return None if val is None or (isinstance(
                            val, float) and pd.isna(val)) else val
