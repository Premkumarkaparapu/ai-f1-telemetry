...
def load_to_db(ff1: fastf1.core.Session, year: int, event: str, stype: str, db: DBSession):
    ...
    for drv_num in ff1.laps["DriverNumber"].dropna().unique():
        ...
        try:
            ...
            code = k; full_name = k; team = ""; team_color = "#888888"
        drv = Driver(session_id=sid, code=code, full_name=full_name, team=team, team_color=team_color)
        db.add(drv)
        db.flush()
    ...
    for _, r in laps_df.iterrows():
        ...
        fc_ms   = int(r["fuel_corrected_lap_time_ms"]) if _safe(r.get("fuel_corrected_lap_time_ms")) is not None else (
        ...
        lap = Lap(
            ...
            sector1_ms   = int(r["sector1_ms"])   if _safe(r.get("sector1_ms"))   is not None else None,
            sector2_ms   = int(r["sector2_ms"])   if _safe(r.get("sector2_ms"))   is not None else None,
            sector3_ms   = int(r["sector3_ms"])   if _safe(r.get("sector3_ms"))   is not None else None,
            compound     = str(r.get("compound", "UNKNOWN")),
            tyre_life    = int(r["tyre_life"])    if _safe(r.get("tyre_life"))    is not None else None,
            stint_number = int(r["stint_number"]) if _safe(r.get("stint_number")) is not None else None,
            is_pit_lap   = bool(r.get("is_pit_lap", False)),
    ...
    # Stints
    try:
        stint_df = ff1.laps[["DriverNumber","Stint","Compound","LapNumber"]].dropna(subset=["Stint"])
    ...
    try:
        ...
        for _, r in pit_laps.iterrows():
            k = str(int(r["DriverNumber"])); did = driver_map.get(k)
            if not did: continue
            dur = None
            ...
            db.add(PitStop(driver_id=did, session_id=sid, lap_number=int(r["LapNumber"]), duration_ms=dur))
    ...
    for drv_num in top_drivers:
        ...
        try:
            ...
            if existing > 0:
                logger.info("    Driver %s: already has %d telemetry pts, skipping", drv_num, existing)
                tel_total += existing
                continue
            ...
            for _, pt in tel_s.iterrows():
                db.add(TelemetryPoint(
                    ...
                    session_id = sid,
                    time_ms    = int(pt["_bin"]),
                    distance_m = float(pt["Distance"])   if _safe(pt.get("Distance"))   is not None else None,
                    ...
                    throttle_pct = float(pt["Throttle"])if _safe(pt.get("Throttle"))   is not None else None,
                    brake = bool(int(pt["Brake"]) > 0) if _safe(pt.get("Brake")) is not None else False,
...
def bulk_load():
    ...
    for s in failed:  print(f"    FAIL {s}")
    print("=" * 55)
