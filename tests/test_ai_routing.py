...
def test_llm_structured_routing_success():
    ...
    mock_ai.complete_json.return_value = {
        "tools": [
            {"tool_name": "get_driver_pace", "driver_code": "VER", "start_lap": 1, "end_lap": 10}
        ]
    }
    
    agent = DataAgent(mock_ai, db=MagicMock())
    selected = agent._select_tools_via_llm("How was VER pace in the opening laps?", driver_code="VER")
