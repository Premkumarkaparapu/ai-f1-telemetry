...
from backend.app.core.config import DATABASE_URL, MODEL_PATH  # noqa: E402
...
def _load_laps() -> pd.DataFrame:
    ...
    engine.dispose()
    logger.info(
        "Loaded %d valid laps from %d sessions",
        len(df),
...
def _build_features(df: pd.DataFrame, track_enc: dict = None, return_enc: bool = False):
    ...
    if track_enc is None:
        track_means = (
            df.groupby("track")["fuel_corrected_lap_time_ms"].mean().sort_values()
        )
        track_enc = {t: i for i, t in enumerate(track_means.index)}
    df["track_enc"] = (
        df["track"].map(track_enc).fillna(len(track_enc) // 2).astype(int)
    )
...
def train_all() -> None:
    ...
    if len(df) < MIN_ROWS:
        logger.warning(
            "Only %d valid laps — skipping training. Ingest data first.",
            len(df),
        )
        out = MODEL_PATH / "compound_means.json"
        with open(out, "w") as f:
    ...
    logger.info("=" * 60)
    logger.info(
        "Training on %d laps | %d compounds | %d tracks | %d sessions",
        len(df),
        df["compound"].nunique(),
        df["track"].nunique(),
        df["session_id"].nunique(),
